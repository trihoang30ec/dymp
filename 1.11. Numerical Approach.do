...
graph twoway (scatter v_k k_, connect(direct) msymbol(x) connect(none) yvarlab("v_n(k) for n > 1") connect(ascending) xtitle("Captial Stock k Today") ytitle("Value Function v(k)") mcolor(black)) ///
(scatter v_k k_ if n == `t_converged', yvarlab("Converged v(k)") connect(none) msymbol(X) connect(none) mcolor(red) connect(ascending)) ///
(function y = `alpha_0' + `alpha_1'*log(x), range(2 10) color(blue) plotregion(style(none)) yvarlab("v(k) = `a_0' + `a_1'*log(k) (analytic solution)"))
